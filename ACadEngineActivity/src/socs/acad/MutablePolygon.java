...
public class MutablePolygon extends Shape
{
	...
	/**
	 * Convert a float into a string 1 unit of the float = 1"
	 */
	...
	/**
	 * Handles the touching of the area and alters the state of the polygon and
	 * its vertices as necessary. This is the heavy lifter of this class, and
	...
	public boolean onAreaTouched(final TouchEvent pSceneTouchEvent, final float pTouchAreaLocalX, final float pTouchAreaLocalY)
	{
		switch (pSceneTouchEvent.getAction())
		{
			case TouchEvent.ACTION_DOWN:
				...
				// If we are in EDIT mode, then determine the context of the
				// touch
				if (polygonState == PolygonState.EDIT)
				{
					...
					if (!vertexTouched && canAddVertex)
					{
						...
						// Do not include the closing edge if the polygon is not
						// closed
			...
			case TouchEvent.ACTION_MOVE:
				if (stateDragging)
				{
					...
					// sensitive to movement, so put a threshold value to
					// protect
					// against making it too difficult to perform a simple
					// touch,
					// and hopefully avoid frustrating the user.

					if (hasMoved || (Math.abs(pSceneTouchEvent.getX() - previousSceneX) > DRAGTHRESHOLD)
							|| (Math.abs(pSceneTouchEvent.getY() - previousSceneY) > DRAGTHRESHOLD))
					{
						hasMoved = true;
						if (polygonState == PolygonState.VIEW)
						{
							...
							// If rotated, then the raw xMoved and yMoved are
							// not
							...
							if (vertexTouchedAgain)
							{
								if (canDeleteVertex)
								{
									...
									} else if (automaticallyDelete)
									{
										...
										// minimumVertexCount, remove it all.
										detach();
									}
								}
							} else
	...
	public void moveVertex(int index, float pX, float pY)
	{
		...
		measurement[precedingPosition] = FloatMath.sqrt(distanceSquared(mVertexX[precedingPosition], mVertexY[precedingPosition], mVertexX[index],
				mVertexY[index]));
	...
	/**
	 * Compute the distance (squared) from a point to a segment. These are
	 * well-known formulas.
	 */
	...
	/**
	 * Determine whether or not two line segments intersect. This is a
	 * well-known solution. http://www.java-gaming.org/index.php?topic=22590.0
	 */
	public boolean segmentsIntersect(float x1, float y1, float x2, float y2, float x3, float y3, float x4, float y4)
	{
		...
		if (commonDenominator == 0)
		{
			...
			if (collinearityTestForP3 == 0)
			{
				...
				if (x1 >= x3 && x1 <= x4 || x1 <= x3 && x1 >= x4 || x2 >= x3 && x2 <= x4 || x2 <= x3 && x2 >= x4 || x3 >= x1 && x3 <= x2 || x3 <= x1
						&& x3 >= x2)
				{
					if (y1 >= y3 && y1 <= y4 || y1 <= y3 && y1 >= y4 || y2 >= y3 && y2 <= y4 || y2 <= y3 && y2 >= y4 || y3 >= y1 && y3 <= y2 || y3 <= y1
							&& y3 >= y2)
					{
						return true;
					}
	...
	/**
	 * Decide if the point is within the polygon. It says that this is
	 * deprecated, but it lies. This method is called by the Scene, so therefore
	 * must exist.
	 */
	@Override
	@Deprecated
	...
	/**
	 * Update the set of vertices that define this polygon. Unfortunately, the
	 * Polygon class is buggy when adding or moving vertices, so this is almost
	 * a complete re-work of the Polygon.updateVertices() code.
	 */
	...
	public void addVertex(int position, float x, float y)
	{
		...
		newMeasurement[precedingPosition] = FloatMath.sqrt(distanceSquared(newVertexX[precedingPosition], newVertexY[precedingPosition], newVertexX[position],
				newVertexY[position]));
		newMeasurement[position] = FloatMath.sqrt(distanceSquared(newVertexX[position], newVertexY[position], newVertexX[nextPosition],
				newVertexY[nextPosition]));
	...
	public void removeVertex(int position)
	{
		...
		newMeasurement[precedingPosition] = FloatMath.sqrt(distanceSquared(newVertexX[precedingPosition], newVertexY[precedingPosition],
				newVertexX[nextPosition], newVertexY[nextPosition]));
	...
	/**
	 * Fix the background color. Unfortunately, a polygon will not show up when
	 * it is first added to the Scene, even if the background color is already
	 * set. It must be set again, so here is a function to do it all in one
	 * place.
	 */
