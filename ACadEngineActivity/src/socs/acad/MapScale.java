...
public class MapScale extends Entity
{
	...
	private void repopulate()
	{
		...
		{
			l = new Line(MARGIN + (i * scaledGridSize), MARGIN, MARGIN + (i * scaledGridSize), MARGIN + (i == 0 || i == parts ? endLineHeight : lineHeight),
					vboManager);
			l.setLineWidth(2);
			this.attachChild(l);
		}
		...
		Text t = new Text(MARGIN + (width / 2), MARGIN + lineHeight + 5, ACadEngineActivity.measurementFont, MutablePolygon.formatDistance(gridIncrement
				* parts), 30, new TextOptions(HorizontalAlign.CENTER), vboManager);
